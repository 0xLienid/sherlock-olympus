# OlympusDAO Contest Details (02/2023)
...
- The address(es) with`liquidityvault_admin` permissions in `SingleSidedLiquidityVault.sol` and `WstethLiquidityVault.sol` are owned by the protocol and is used to add/remove reward tokens, update configurations, and rescue funds in-case of emergency.
- All functionality in both the Minter and Burner contracts are gated by the OlympusV3 role-based access controls (see OlympusRoles.sol). Specifically, an address must have the `minter_admin` role to execute transactions on the Minter contract and the `burner_admin` role to execute transactions on the Burner contract. The address(es) with these permissions are owned by the protocol.
